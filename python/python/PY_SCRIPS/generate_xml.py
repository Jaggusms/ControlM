...
for i in ['CYCLIC','MAXWAIT','MAXRERUN']:
    XML_DATA[i]=XML_DATA[i].apply(lambda x:x if x!="" else 0)
...
for idx,row in XML_DATA[(XML_DATA.TAG=='FOLDER') |(XML_DATA.TAG=='SMART_FOLDER') ].iterrows():
    ...
    MAXWAIT_value=list(JOB_data['MAXWAIT'])[0]
    ...
    if len(SUB_folder_data)!=0:
        for idx, row in SUB_folder_data.iterrows():
            ...
            for cond in PRED_ON_data:
                ...
                PRED_ON_DATA_tag.set('CODE','COMPSTAT EQ 0')
                DOCOND_tag=ET.Element('DOCOND')
                DOCOND_tag.set('NAME',SUB_ROW[1]+"-TO-"+cond.replace("/","-").replace(" ",""))
                DOCOND_tag.set('ODATE','ODAT')
                DOCOND_tag.set('SIGN','+')
                PRED_ON_DATA_tag.append(DOCOND_tag)
                second_level_tag.append(PRED_ON_DATA_tag)
        ...
        for idx, row in JOB_data.iterrows():
            ...
            for data in INCOND_DATA:
                ...
                if "/" in data:
                    ...
                    INCOND_DATA_tag.set('NAME',data[0].replace(" ","")+"-TO-"+row[3])
                INCOND_DATA_tag.set('ODATE',data[1])
                INCOND_DATA_tag.set('AND_OR','A')
                OUTCOND_DATA_tag=ET.Element('OUTCOND')
                ...
                if "/" in data:
                    ...
                    OUTCOND_DATA_tag.set('NAME',data[0].replace(" ","")+"-TO-"+row[3])
            ...
            for data in OUTCOND_DATA:
                ...
                if "/" in data:
                    ...
                    OUTCOND_DATA_tag.set('NAME',row[3]+"-TO-"+data.replace(" ",""))
            ...
            if EVENT_ON_DATA.upper()=='YES':
                ...
                EVENT_ON_DATA_tag.append(DOACTION_tag)
                JOB.append(EVENT_ON_DATA_tag)
            MAIL_ON_DATA=",".join(row[32].split("\n"))
            if MAIL_ON_DATA!="":
                MAIL_ON_DATA_tag=ET.Element('ON')
                MAIL_ON_DATA_tag.set('STMT','*')
                if eval(MAIL_ON_DATA).get("STATUS","").upper()=="1":
                    MAIL_ON_DATA_tag.set('CODE','OK')
                else:
                     MAIL_ON_DATA_tag.set('CODE','NOTOK')
                DOMAIL_tag=ET.Element('DOMAIL')
                DOMAIL_tag.set('URGENCY','R')
                DOMAIL_tag.set('DEST',eval(MAIL_ON_DATA).get("TO",""))
                DOMAIL_tag.set('CC_DEST',eval(MAIL_ON_DATA).get("CC",""))
                DOMAIL_tag.set('SUBJECT',eval(MAIL_ON_DATA).get("SUBJECT",""))
                DOMAIL_tag.set('MESSAGE',"0001"+eval(MAIL_ON_DATA).get("MESSAGE",""))
                DOMAIL_tag.set('ATTACH_SYSOUT',eval(MAIL_ON_DATA).get("ATTACH_OUTPUT",""))
                MAIL_ON_DATA_tag.append(DOMAIL_tag)
                JOB.append(MAIL_ON_DATA_tag)
