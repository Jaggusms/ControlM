...
def out(x,name):
    ...
    appl_ENQUEUE=[]
    APPL_HOLD=""
    for line in x[:folder_lines[1] if len(folder_lines)>1 else len(x) ]:
...
#xml_Folder_Control.to_excel("conform_folder.xlsx",sheet_name="asdf",index=False)
...
#xml_jobs_control.to_excel("xml_jobs_control.xlsx",sheet_name="asdf",index=False)
...
#xml_Folder_Confirm.to_excel("xml_Folder_Confirm.xlsx",sheet_name="ESP_JOBS_CR",index=False)
...
#xml_jobs_Confirm_data.to_excel("xml_jobs_Confirm_data.xlsx",sheet_name="ESP_JOBS_CR",index=False)
