...
def send_email(sender_email,mail_to,mail_sub,body,mail_cc=[],attachments=[]):
    ...
    try:
        ...
        print("Mail Id's Invalid or Organization VPN not connected")
        logging.shutdown()
        script_log_moving()
        sys.exit(1)
...
def emails():
    ...
    mail_cc=['Srikanth.Gadekal@legato.com']
    ...
    for idx,rows in Migatrion_file.iterrows():
        try:
            if not rows[COLUMN[1]].split(".")[-2].startswith(rows[COLUMN[0]][-3]) or  not rows[COLUMN[2]].split(".")[-2].startswith(rows[COLUMN[0]][-1]):
                ...
                print(f"{rows_data} source and target Environments not matched with Promition {rows[COLUMN[0]]}")
                validation_success=False
        except:
            rows_data=",".join(rows)
            failure_data.append(rows_data)
            ...
            rows_data=",".join(rows)
            print(f"{rows_data} source and target Environments matched with Promition {rows[COLUMN[0]]}")
        if "".join(rows) not in cros_work_file_data:
            rows_data=",".join(rows)
            failure_data.append(rows_data)
            print(f"{rows_data} not matched with Cross work File")
            validation_success=False      
        else:
            rows_data=",".join(rows)
            print(f"{rows_data} matched with Cross work File")
    return validation_success,failure_data
...
def parameter_validation():
    if len(sys.argv[1:])!=1:
        ...
        print("Parameter taken Successfull")
        ...
        if validation_sucess:
            ...
            for data,row in df2.iterrows():
                if row[2].upper()=="TRANSFER":
                    EVENT_NAME,SourcCrosswalk,environment_1,SCHEDULE=row[0],row[3],row[4],row[5]
                    ...
                    if SCHEDULE.upper()=="AUTO":
                        s=ASCII_REMOVAL('zowe endevor transfer element {1} --env SUPP --sys {4} --sub {2} --typ EVENTS --sn 2 --ccid {0} --com "DEFAULT AUTO" --toenv SUPP --tosys {4} --tosub {3} --toele {1} --totyp EVENTS --tosn 2 --sync --pg EVENTS --i ENDEVOR –bed'.format(RITM,EVENT_NAME,environment,environment_1,application))									  
                        commands.append(s)
                    else:
                        s=ASCII_REMOVAL('zowe endevor transfer element {1} --env SUPP --sys {4} --sub {2} --typ EVENTS --sn 2 --ccid {0} --com "DEFAULT" --toenv SUPP --tosys {4} --tosub {3} --toele {1} --totyp EVENTS --tosn 2 --sync --pg EVENTS --i ENDEVOR –bed'.format(RITM,EVENT_NAME,environment,environment_1,application))									  
                        commands.append(s)

                if row[2].upper()=="UPDATE":
                    appl,SourcCrosswalk,SCHEDULE,Promotion=row[0],row[3],row[5],row[1]
                    environment=SourcCrosswalk.split(".")[4]
                    application=SourcCrosswalk.split(".")[3]
                    if SCHEDULE.upper()=="AUTO":
                        s=ASCII_REMOVAL('zowe endevor update element {1} --env SUPP --sys {3} --sub {4} --typ EVENTS --fd {2} --fm {1} --ccid Jira123 --com "{5} file add AUTO" --i ENDEVOR –g'.format(RITM,appl,SourcCrosswalk,application,environment,Promotion))
                        commands.append(s)
                    
                    else:
                        s=ASCII_REMOVAL('zowe endevor update element {1} --env SUPP --sys {3} --sub {4} --typ EVENTS --fd {2} --fm {1} --ccid Jira123 --com "{5} file add" --i ENDEVOR –g'.format(RITM,appl,SourcCrosswalk,application,environment,Promotion))
                        commands.append(s)
                ...
                if row[2].upper()=="ADD":
                    ...
                    application=SourcCrosswalk.split(".")[3]
                    if SCHEDULE.upper()=="AUTO":
                        s=ASCII_REMOVAL('zowe endevor add element {1} --env SUPP --sys {3} --sub {4} --typ EVENTS --fd {2} --fm {1} --ccid Jira123 --com "{5} file add AUTO" --i ENDEVOR –g'.format(RITM,appl,SourcCrosswalk,application,environment,Promotion))
                        commands.append(s)
                    
                    else:
                        s=ASCII_REMOVAL('zowe endevor add element {1} --env SUPP --sys {3} --sub {4} --typ EVENTS --fd {2} --fm {1} --ccid Jira123 --com "{5} file add" --i ENDEVOR –g'.format(RITM,appl,SourcCrosswalk,application,environment,Promotion))
                        commands.append(s)
                ...
                if row[2].upper()=="MOVE":
                    ...
                    application=SourcCrosswalk.split(".")[3]
                    if SCHEDULE.upper()=="AUTO":
                        s=ASCII_REMOVAL('zowe endevor move element {1} --env SUPP --sys {2} --sub {3} --typ EVENTS --sn 2 --ccid {0} --com "DEFAULT AUTO" --bed'.format(RITM,appl,application,environment_1)) 
                        commands.append(s)

                    else:
                        s=ASCII_REMOVAL('zowe endevor move element {1} --env SUPP --sys {2} --sub {3} --typ EVENTS --sn 2 --ccid {0} --com "DEFAULT" --bed'.format(RITM,appl,application,environment_1)) 
                        commands.append(s)

                    
            commands=[i.replace(i[4]," ") for i in commands]
            ...
            print("Validation Fail going to send the Mail with Failure Data")
            ...
            body=str("\n".join([ i for i in [file]+["Validation Failure Data as follows\n"]+["Please check your Migration Form"]]))
