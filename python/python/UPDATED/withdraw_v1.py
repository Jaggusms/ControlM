...
appls_df=dfe[dfe.Lines.str.contains("./ ADD    NAME=")]
...
esp_df1=[]
for name in l:
    APPL_=APPL_DETAILS(original_,name,dictonary_with)
    if len(APPL_[1])!=2:
...
esp_df[esp_df.columns[0]]=esp_df[esp_df.columns[0]].apply(lambda x:x.upper())
esp_df1[esp_df1.columns[0]]=esp_df1[esp_df1.columns[0]].apply(lambda x:x.upper())
esp_df1[esp_df1.columns[1]]=esp_df1[esp_df1.columns[1]].apply(lambda x:x.upper())
