...
d_folder,d_job=perticular_details(df1_old,df2_old)
df1_old = pd.DataFrame(columns=folder[0], data=d_folder)
df2_old = pd.DataFrame(columns=job[0], data=d_job)
...
d_folder,d_job=perticular_details(df1_new,df2_new)
df1_new = pd.DataFrame(columns=folder[0], data=d_folder)
df2_new = pd.DataFrame(columns=job[0], data=d_job)                                   
...
with pd.ExcelWriter(output) as writer:
    new_folder.to_excel(writer, sheet_name='FOLDER',index=False)
    new_JOB.to_excel(writer, sheet_name='JOB',index=False)       
import openpyxl
...
wb.save(output)
miss=[i for i in list(set(miss_folder)) if i not in list(new_folder['FOLDER_NAME'])]
import os
