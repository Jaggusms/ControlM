    ...
    if len(row[2])==0:
        ...
        for in_cond in row[2]:
            ...
            if len(new_data.index)==0:
                ...
                predessor2.append("\n".join([i[0]  if i[0]==i[1] else "-".join(i) if row[0]==i[0] else i[1]+"("+i[0]+")"  for i in new_data[['PARENT_FOLDER','JOBNAME']].values.tolist()]))
                ...
                if new_data['TAG'].tolist()[0]=='JOB':
                    ...
                    predessor1.append("\n".join(new_data['JOBNAME'].tolist()))
                Matched_In_Out_Conditions_index.append(i)
        if a:
            Not_exist_OutConditions.append("\n".join(Not_exist))
        if b:
            #only_jobs.append(",".join(jobs))
            Same_In_Out_in_different_folder.append("\n".join(predessor2))
            Same_In_Out_details.append("\n".join(Same_In_Out_))
        if c:
            Matched_In_Out_Conditions.append("\n".join(list(set(predessor1))))
...
Same_In_Out["which condition"]=Same_In_Out_details
