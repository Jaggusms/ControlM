...
if len(sys.argv)!=4:
    print("please pass correct params")
    sys.exit(1)
xml_file,imp_jobs_file,appl_rename_file=sys.argv[1],sys.argv[2],sys.argv[3]
if not (xml_file.lower().endswith("xml") or imp_jobs_file.lower().endswith(".xlsx") or appl_rename_file.lower().endswith(".xlsx")):
    print("Please pass valide params 1. xml_file 2. IMpacted file 3. Appl Rename File")
    sys.exit(1)

print("Program executing Please wait!")
start=datetime.now()
class conditions_Checking:
    def __init__(self,xml_file,imp_jobs_file,appl_rename_file):
...
object=conditions_Checking(xml_file,imp_jobs_file,appl_rename_file)
...
def convert_to_string(df):
    ...
    return df
matched_data=data.iloc[list(set(Matched_In_Out_Conditions_index)),[0,1,2,6]]
matched_data['predessor']=Matched_In_Out_Conditions
Not_exist=data.iloc[list(set(Not_exist_OutConditions_index)),[0,1,2,6]]
Not_exist['not_exist_condition']=Not_exist_OutConditions
Same_In_Out=data.iloc[list(set(Same_In_Out_in_different_folder_index)),[0,1,2,6]]
...
new_matched_data=matched_data[matched_data.columns[[0,1,2,4]]]
Impacted=object.imp_jobs_data()
new_matched_data['new_jobs_with_Impacted']=impact_matching(Impacted,matched_data,indexes=[0,2])

appl_rename_updated=object.appl_rename_data()
appl_rename_columns=appl_rename_updated.columns
appl_rename_updated=appl_rename_updated.rename(columns={appl_rename_columns[0]:"PARENT_FOLDER",appl_rename_columns[1]:"APPL_NAME"})
#print(appl_rename_updated.iloc[:5,:])

new_matched_data=pd.merge(new_matched_data,appl_rename_updated, on="PARENT_FOLDER",how="left").fillna("")

new_matched_data=new_matched_data[new_matched_data.predessor!=""]

def impact_matching(Impacted,appl_rename_updated,new_matched_data,index=[0,1]):
    new_jobs_with_impacted=[]
    impd=dict(zip(list(Impacted[Impacted.columns[3]]),list(Impacted[Impacted.columns[2]])))
    appld=dict(zip(list(appl_rename_updated['PARENT_FOLDER']),list(appl_rename_updated['APPL_NAME'])))
    for idx,row in new_matched_data[new_matched_data.columns[index]].iterrows():
        if "," in row[1]:
            l=[]
            for i in row[1].split(","):
                if "(" in i:
                    folder_name=i[i.find("(")+1:-1]
                    job_name=i[:i.find("(")]
                    l.append(impd.get(folder_name+job_name,job_name)+"("+appld.get(folder_name,folder_name)+")")
                else:
                    v=impd.get(row[0]+i,i)
                    l.append(appld.get(v,v))
            new_jobs_with_impacted.append(",".join(l))
        else:
            if "(" in row[1]:
                folder_name=row[1][row[1].find("(")+1:-1]
                job_name=row[1][:row[1].find("(")]
                new_jobs_with_impacted.append(impd.get(folder_name+job_name,job_name)+"("+appld.get(folder_name,folder_name)+")")
            else:
                v=impd.get(row[0]+row[1],row[1])
                new_jobs_with_impacted.append(appld.get(v,v))
    return new_jobs_with_impacted
new_matched_data['new_predessor_with_Impacted']=impact_matching(Impacted,appl_rename_updated,new_matched_data,index=[0,3])
def folder_to_appl(data,appl_rename_updated):
    appld=dict(zip(list(appl_rename_updated['PARENT_FOLDER']),list(appl_rename_updated['APPL_NAME'])))
    new_jobs_with_appls=[]
    for i in data:
        new_jobs_with_appls.append(appld.get(i,i))
    return new_jobs_with_appls
new_matched_data['new_jobs_with_Impacted']=folder_to_appl(list(new_matched_data['new_jobs_with_Impacted']),appl_rename_updated)
new_matched_data=convert_to_string(new_matched_data).drop_duplicates().reset_index(drop=True)
...
with pd.ExcelWriter("CTM_In-Out_Conditions"+str(datetime.now().strftime("%H%M%S"))+".xlsx") as w:
    ...
    new_matched_data.to_excel(w, sheet_name='new_matched_data',index=False)
