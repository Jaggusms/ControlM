...
if not (xml_file.lower().endswith(".xml") or esp_file.lower().endswith(".txt") or imp_jobs_file.lower().endswith(".xlsx") or appl_rename_file.lower().endswith(".xlsx")):
    ...
    for i,line in enumerate(x):
        ...
        if job_type in JOB_TYPES:
            ...
            if "IF DAYS_TO" in jobs_string:
                ...
                jobs_date.append("")            
    folder_rbcname_date={} 
    if_days_string="".join(x[:-1 if len(idx)==0 else idx[0]]) 
...
def data(l,original,dictonary_with):
    for name in l:
        ...
        for idx,jobs in enumerate(APPL_[0]):
            #print([name,jobs,APPL_[2][idx]])
            jobs_esp_data.append([name,jobs,APPL_[2][idx]])
#l=['EPMWN0BZ','EPMWF0BZ','EPMWE0BZ','EPMWC0BZ','EPPEDW29','EPPEDW19']
#esp data reading 
...
xml_jobs_till_esp["APPL_NAME"]=[a if a==b else a+b for a,b in zip(xml_jobs_till_esp[xml_jobs_till_esp.columns[4]],xml_jobs_till_esp[xml_jobs_till_esp.columns[5]])]
